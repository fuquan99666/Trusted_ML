...
class NoisyBatchNorm2d(nn.BatchNorm2d):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
        ...
        self.neuron_mask = Parameter(torch.Tensor(num_features))        ### Mask parameters on weights
        self.neuron_noise = Parameter(torch.Tensor(num_features))       ### Noise parameters on weights
        self.neuron_noise_bias = Parameter(torch.Tensor(num_features))  ### Noise parameters on bias
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if self.is_perturbed:
            ###################################################
            ## When we need to perturb the nodes, how to define the coeff_weights and coeff_bias
...
class NoisyBatchNorm1d(nn.BatchNorm1d):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True):
        super(NoisyBatchNorm1d, self).__init__(
            num_features, eps, momentum, affine, track_running_stats)
        self.neuron_mask_fc = Parameter(torch.Tensor(num_features))         ### Mask parameters on weights
        self.neuron_noise_fc = Parameter(torch.Tensor(num_features))        ### Noise parameters on weights
        self.neuron_noise_bias_fc = Parameter(torch.Tensor(num_features))   ### Noise parameters on bias
